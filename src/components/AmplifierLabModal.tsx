...
export const AmplifierLabModal: React.FC<AmplifierLabModalProps> = ({ onClose }) => {
  ...
  // Full Screen State for Laboratory Viewing
  const [isFullScreen, setIsFullScreen] = useState<boolean>(true);

  // Practical 2 Slow Motion & Frequency Controls for Current Flow
  const [p2AnimSpeed, setP2AnimSpeed] = useState<number>(0.2); // 0.05=0.1Hz (Ultra Slow), 0.2=0.5Hz (Slow), 0.5=1Hz (Standard), 2.0=10Hz (Fast)
  const [p2IsPaused, setP2IsPaused] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (!isLiveAnim || (selectedPracticalNo === 2 && p2IsPaused)) return;
    const step = selectedPracticalNo === 2 ? Math.max(1, Math.round(15 * p2AnimSpeed)) : 15;
    const timer = setInterval(() => {
      setAnimOffset((prev) => (prev + step) % 1000);
    }, 40);
    return () => clearInterval(timer);
  }, [isLiveAnim, p2IsPaused, p2AnimSpeed, selectedPracticalNo]);
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Header Action Buttons (Full Screen Toggle & Close) */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <button
              ...
              style={{
                ...
                borderRadius: '8px',
                border: '1px solid var(--border-color)',
                background: isFullScreen ? 'rgba(56, 189, 248, 0.15)' : 'var(--bg-card)',
                color: isFullScreen ? '#38bdf8' : 'var(--text-secondary)',
                fontSize: '0.74rem',
                fontWeight: 700,
                cursor: 'pointer',
              }}
              title={isFullScreen ? 'Exit Full Screen Window Mode' : 'Expand to Full Screen Mode'}
            >
        ...
        {activeTab === 'circuit' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', flex: 1, overflowY: 'auto' }}>
            ...
            {/* Practical 2 Slow Motion Frequency & Playback Control Bar */}
            {selectedPracticalNo === 2 && (
              <div style={{ background: 'var(--bg-card)', padding: '10px 14px', borderRadius: '10px', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                  {/* Frequency / Slow Motion Preset Buttons */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: '0.74rem', fontWeight: 800, color: '#f43f5e', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <Sparkles size={13} /> Slow Motion / Speed:
                    </span>
                    {[
                      { label: '🐢 0.1 Hz (Ultra Slow)', speed: 0.05 },
                      { label: '🐢 0.5 Hz (Slow Mo)', speed: 0.2 },
                      { label: '⚡ 1 Hz (Demo)', speed: 0.5 },
                      { label: '🚀 10 Hz (Fast)', speed: 2.0 },
                      { label: '⚡ 1 kHz (Real)', speed: 6.0 },
                    ].map((opt) => (
                      <button
                        key={opt.label}
                        onClick={() => {
                          setP2AnimSpeed(opt.speed);
                          setP2IsPaused(false);
                        }}
                        style={{
                          padding: '3px 8px',
                          borderRadius: '6px',
                          border: `1px solid ${p2AnimSpeed === opt.speed && !p2IsPaused ? '#f43f5e' : 'var(--border-color)'}`,
                          background: p2AnimSpeed === opt.speed && !p2IsPaused ? 'rgba(244, 63, 94, 0.2)' : 'var(--bg-panel)',
                          color: p2AnimSpeed === opt.speed && !p2IsPaused ? '#f43f5e' : 'var(--text-secondary)',
                          fontSize: '0.7rem',
                          fontWeight: 700,
                          cursor: 'pointer',
                        }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <button
                      onClick={() => setP2IsPaused(!p2IsPaused)}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '4px',
                        padding: '4px 10px',
                        borderRadius: '6px',
                        border: `1px solid ${p2IsPaused ? '#f59e0b' : '#4ade80'}`,
                        background: p2IsPaused ? 'rgba(245, 158, 11, 0.2)' : 'rgba(74, 222, 128, 0.2)',
                        color: p2IsPaused ? '#f59e0b' : '#4ade80',
                        fontSize: '0.72rem',
                        fontWeight: 800,
                        cursor: 'pointer',
                      }}
                    >
                      {p2IsPaused ? <Play size={12} /> : <Pause size={12} />}
                      <span>{p2IsPaused ? 'RESUME PLAY' : 'PAUSE / FREEZE'}</span>
                    </button>

                    <button
                      onClick={() => {
                        setP2IsPaused(true);
                        setAnimOffset((prev) => (prev - 42 + 1000) % 1000);
                      }}
                      style={{
                        padding: '4px 8px',
                        borderRadius: '6px',
                      ...
                      title="Step back 15 degrees in waveform cycle"
                    >
                      ⏮️ -15° Step
                    </button>
                    ...
                    <button
                      onClick={() => {
                        setP2IsPaused(true);
                        setAnimOffset((prev) => (prev + 42) % 1000);
                      }}
                      style={{
                        padding: '4px 8px',
                        borderRadius: '6px',
                      ...
                      title="Step forward 15 degrees in waveform cycle"
                    >
                      ⏭️ +15° Step
                    </button>
                  </div>
                ...
                {/* Waveform Cycle Interactive Scrubber Slider */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'var(--bg-panel)', padding: '6px 12px', borderRadius: '8px' }}>
                  <span style={{ fontSize: '0.72rem', fontWeight: 800, color: 'var(--accent-cyan)', whiteSpace: 'nowrap' }}>
                    🔄 AC Cycle Phase Scrubber:
                  </span>
                  <input
                    type="range"
                    min="0"
                    max="999"
                    value={animOffset % 1000}
                    onChange={(e) => {
                      setP2IsPaused(true);
                      setAnimOffset(parseInt(e.target.value, 10));
                    }}
                    style={{ flex: 1, accentColor: '#f43f5e', cursor: 'pointer' }}
                  />
                  <span style={{ fontSize: '0.72rem', fontWeight: 900, color: '#fff', width: '140px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                    Phase: {Math.round(((animOffset % 1000) / 1000) * 360)}° ({(params.vinPeak * Math.sin(animOffset * 0.08)) > 0 ? '+' : ''}${(params.vinPeak * Math.sin(animOffset * 0.08)).toFixed(2)}V)
                  </span>
                </div>
              </div>
            )}
                  ...
                  return (
                    <g>
                      ...
                      <g transform="translate(160, 8)">
                        {isP2Deadzone ? (
                          <g>
                            <rect x="0" y="0" width="410" height="24" fill="rgba(245, 158, 11, 0.25)" stroke="#f59e0b" strokeWidth="1.5" rx="6" />
                            <text x="12" y="16" fill="#f59e0b" fontSize="10" fontWeight="900">
                              ⚠️ DEADZONE (|Vin|={Math.abs(instantVin).toFixed(2)}V &lt; {p2VbeCutoff}V) — Q1 OFF (IB1=0) | Q2 OFF (IB2=0)
                            </text>
                          </g>
                        ) : isQ1Active ? (
                          <g>
                            <rect x="0" y="0" width="410" height="24" fill="rgba(244, 63, 94, 0.25)" stroke="#f43f5e" strokeWidth="1.5" rx="6" />
                            <text x="12" y="16" fill="#f43f5e" fontSize="10" fontWeight="900">
                              ⚡ POSITIVE HALF (+Vin = +{instantVin.toFixed(2)}V) — Q1 NPN ON (180°) 🔥 | Q2 PNP OFF 💤
                            </text>
                          </g>
                        ) : (
                          <g>
                            <rect x="0" y="0" width="410" height="24" fill="rgba(74, 222, 128, 0.25)" stroke="#4ade80" strokeWidth="1.5" rx="6" />
                            <text x="12" y="16" fill="#4ade80" fontSize="10" fontWeight="900">
                              ⚡ NEGATIVE HALF (-Vin = {instantVin.toFixed(2)}V) — Q2 PNP ON (180°) 🔥 | Q1 NPN OFF 💤
                            </text>
                          </g>
                        )}
                      </g>
                      ...
                      <g onMouseEnter={() => setHoveredCompId('v1')} onMouseLeave={() => setHoveredCompId(null)} style={{ cursor: 'pointer' }}>
                      ...
                      <g opacity={isQ1Active ? 1 : 0.45} onMouseEnter={() => setHoveredCompId('c1')} onMouseLeave={() => setHoveredCompId(null)} style={{ cursor: 'pointer' }}>
                      ...
                      <g opacity={isQ2Active ? 1 : 0.45} onMouseEnter={() => setHoveredCompId('c2')} onMouseLeave={() => setHoveredCompId(null)} style={{ cursor: 'pointer' }}>
                      ...
                      {/* Sub-branch Base Current Vector Arrow Ib1 */}
                      {isQ1Active && (
                        <g>
                          <polygon points="280,90 270,85 270,95" fill="#f43f5e" />
                          <text x="235" y="84" fill="#f43f5e" fontSize="8" fontWeight="900">➔ Ib1 (+0.2mA)</text>
                        </g>
                      )}
                      ...
                      {/* Sub-branch Base Current Vector Arrow Ib2 */}
                      {isQ2Active && (
                        <g>
                          <polygon points="270,210 280,205 280,215" fill="#4ade80" />
                          <text x="235" y="224" fill="#4ade80" fontSize="8" fontWeight="900">➔ Ib2 (-0.2mA)</text>
                        </g>
                      )}

                      {/* Q1: NPN Transistor 2N3904 (DYNAMIC ACTIVE/INACTIVE VISUAL) */}
                      <g opacity={isQ1Active ? 1 : 0.4} onMouseEnter={() => setHoveredCompId('q1')} onMouseLeave={() => setHoveredCompId(null)} style={{ cursor: 'pointer' }}>
                        {/* Q1 Status Badge */}
                        <rect x="275" y="16" width="110" height="16" fill={isQ1Active ? 'rgba(244,63,94,0.9)' : 'rgba(30,41,59,0.8)'} rx="3" />
                        <text x="282" y="28" fill={isQ1Active ? '#fff' : '#94a3b8'} fontSize="8" fontWeight="900">
                      ...
                      <g opacity={isQ2Active ? 1 : 0.4} onMouseEnter={() => setHoveredCompId('q2')} onMouseLeave={() => setHoveredCompId(null)} style={{ cursor: 'pointer' }}>
                        <circle
                          cx="330"
                          cy="210"
                          r="24"
                        ...
                        <text x="312" y="244" fill={isQ2Active ? '#4ade80' : '#94a3b8'} fontSize="11" fontWeight="800">
                          Q2 2N3906 (PNP)
                        </text>

                        {/* Q2 Status Badge */}
                        <rect x="275" y="254" width="110" height="16" fill={isQ2Active ? 'rgba(74,222,128,0.9)' : 'rgba(30,41,59,0.8)'} rx="3" />
                        <text x="282" y="266" fill={isQ2Active ? '#000' : '#94a3b8'} fontSize="8" fontWeight="900">
                          {isQ2Active ? '🔥 ACTIVE (ON)' : '💤 CUTOFF (OFF)'}
                        </text>
                      </g>

                      {/* Output Junction & Coupling Cap C3 */}
                      <g onMouseEnter={() => setHoveredCompId('c3')} onMouseLeave={() => setHoveredCompId(null)} style={{ cursor: 'pointer' }}>
                      ...
                      {/* Load Resistor RLout */}
                      <g onMouseEnter={() => setHoveredCompId('rl')} onMouseLeave={() => setHoveredCompId(null)} style={{ cursor: 'pointer' }}>
                        ...
                        <text x="535" y="222" fill={isQ1Active ? '#f43f5e' : (isQ2Active ? '#4ade80' : '#64748b')} fontSize="8" fontWeight="900">
                          {isQ1Active ? '⬇️ IL (+18mA Drive)' : (isQ2Active ? '⬆️ IL (-18mA Sink)' : '⏹️ IL (0mA Cutoff)')}
                        </text>
                      </g>
                      ...
                      {/* INTERACTIVE HOVER COMPONENT INSPECTOR OVERLAY */}
                      {hoveredCompId === 'q1' && (
                        <g transform="translate(370, 45)">
                          <rect x="0" y="0" width="220" height="78" fill="rgba(15, 23, 42, 0.95)" stroke="#f43f5e" strokeWidth="2" rx="6" />
                          <text x="10" y="16" fill="#f43f5e" fontSize="10" fontWeight="900">Q1: 2N3904 Complementary NPN BJT</text>
                          <text x="10" y="32" fill="#fff" fontSize="8" fontWeight="700">State: {isQ1Active ? '🔥 ACTIVE CONDUCTING (ON)' : '💤 CUTOFF (IB1 = 0)'}</text>
                          <text x="10" y="46" fill="var(--text-muted)" fontSize="8">Vbe: +0.70V | Ic: {isQ1Active ? `${(Math.max(0, instantVin - p2VbeCutoff) / p2Rout * 1000).toFixed(1)} mA` : '0.0 mA'}</text>
                          <text x="10" y="60" fill="#38bdf8" fontSize="8">Role: Pushes +Vcc power into load RL (Upper 180°)</text>
                        </g>
                      )}

                      {hoveredCompId === 'q2' && (
                        <g transform="translate(370, 165)">
                          <rect x="0" y="0" width="220" height="78" fill="rgba(15, 23, 42, 0.95)" stroke="#4ade80" strokeWidth="2" rx="6" />
                          <text x="10" y="16" fill="#4ade80" fontSize="10" fontWeight="900">Q2: 2N3906 Complementary PNP BJT</text>
                          <text x="10" y="32" fill="#fff" fontSize="8" fontWeight="700">State: {isQ2Active ? '🔥 ACTIVE CONDUCTING (ON)' : '💤 CUTOFF (IB2 = 0)'}</text>
                          <text x="10" y="46" fill="var(--text-muted)" fontSize="8">Vbe: -0.70V | Ic: {isQ2Active ? `${(Math.max(0, -instantVin - p2VbeCutoff) / p2Rout * 1000).toFixed(1)} mA` : '0.0 mA'}</text>
                          <text x="10" y="60" fill="#38bdf8" fontSize="8">Role: Sinks current from load RL to GND (Lower 180°)</text>
                        </g>
                      )}

                      {hoveredCompId === 'c1' && (
                        <g transform="translate(140, 45)">
                          <rect x="0" y="0" width="180" height="42" fill="rgba(15, 23, 42, 0.95)" stroke="#38bdf8" strokeWidth="2" rx="6" />
                          <text x="10" y="16" fill="#38bdf8" fontSize="10" fontWeight="900">C1: 1µF Input Coupling Cap</text>
                          <text x="10" y="30" fill="var(--text-muted)" fontSize="8">Passes positive AC cycle to Q1 base.</text>
                        </g>
                      )}
                      ...
                      {hoveredCompId === 'c2' && (
                        <g transform="translate(140, 205)">
                          <rect x="0" y="0" width="180" height="42" fill="rgba(15, 23, 42, 0.95)" stroke="#38bdf8" strokeWidth="2" rx="6" />
                          <text x="10" y="16" fill="#38bdf8" fontSize="10" fontWeight="900">C2: 1µF Input Coupling Cap</text>
                          <text x="10" y="30" fill="var(--text-muted)" fontSize="8">Passes negative AC cycle to Q2 base.</text>
                        </g>
                      )}

                      {hoveredCompId === 'c3' && (
                        <g transform="translate(360, 100)">
                          <rect x="0" y="0" width="190" height="42" fill="rgba(15, 23, 42, 0.95)" stroke="#f59e0b" strokeWidth="2" rx="6" />
                          <text x="10" y="16" fill="#f59e0b" fontSize="10" fontWeight="900">C3: 100µF Output Coupling Cap</text>
                          <text x="10" y="30" fill="var(--text-muted)" fontSize="8">Blocks DC offset, passes AC to load RL.</text>
                        </g>
                      )}

                      {hoveredCompId === 'rl' && (
                        <g transform="translate(480, 210)">
                          <rect x="0" y="0" width="190" height="52" fill="rgba(15, 23, 42, 0.95)" stroke="#f43f5e" strokeWidth="2" rx="6" />
                          <text x="10" y="16" fill="#f43f5e" fontSize="10" fontWeight="900">RL: {p2Rout}Ω Load Resistor</text>
                          <text x="10" y="30" fill="#fff" fontSize="8">Po: {((Math.pow(params.vinPeak * Math.sin(animOffset * 0.08), 2) / 2) / p2Rout).toFixed(3)} W</text>
                          <text x="10" y="44" fill="var(--text-muted)" fontSize="8">Drives audio speaker / transducer output.</text>
                        </g>
                      )}
                    </g>
                  );
